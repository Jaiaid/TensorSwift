...
cudadev::CUDADev::CUDADev(size_t sizebytes, int deviceid):Device(device_id)
{
    cudaSetDevice(this->device_id);
    cudaMalloc(&this->devbuffer_ptr, sizebytes);
...
{
    cudaSetDevice(this->device_id);
    cudaMemcpy(dst_ptr, this->devbuffer_ptr, this->sizebytes, cudaMemcpyDeviceToHost);
}

void cudadev::CUDADev::write(void *src_ptr)
{
    cudaSetDevice(this->device_id);
    cudaMemcpy(this->devbuffer_ptr, src_ptr, this->sizebytes, cudaMemcpyHostToDevice);
}